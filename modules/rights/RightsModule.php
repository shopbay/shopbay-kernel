...
/**
 * Customization:
 * @see Search for "//---- BEGIN CUSTOMIZATION ----" in mentioned modified files
 * @see Search for "//---- BEGIN BUGFIX ----" in mentioned modified files
 *
 * [1] RightsModule: Added code snippet to support admin css
 * [2] AssignmentController: Fixed bug to display flash message
 * [3] AuthItemController: Fixed bug to display flash message
 * [4] Make AssignmentController to extend from SController rather than RController
 * [5] Add WelcomeFilter to AssignmentController
 * [6] Make AssignmentController to use SAssignmentDataProvider instead of RAssignmentDataProvider
 * [7] Make AssignmentController and AuthItemController extends SController and do a parent::init() to get corresponding layout settings
 *
 * @author kwlok
 *
 **********************************************
...
class RightsModule extends CWebModule
{
	...
	*/
	public function registerScripts()
	{
        ...
        //---- BEGIN CUSTOMIZATION ----
        //register common.css, and application.css
        $assetsUrl = Yii::app()->assetManager->publish(Yii::getPathOfAlias('common.assets.css'));
        Yii::app()->clientScript->registerCssFile($assetsUrl.'/common.css');
        $assetsUrl = Yii::app()->assetManager->publish(Yii::getPathOfAlias('application.assets.css'));
        Yii::app()->clientScript->registerCssFile($assetsUrl.'/application.css');
        $assetsUrl=Yii::app()->assetManager->publish(Yii::getPathOfAlias('common.assets.font-awesome'));
        Yii::app()->clientScript->registerCssFile($assetsUrl.'/css/font-awesome.min.css');
        //---- END CUSTOMIZATION -----
    }
